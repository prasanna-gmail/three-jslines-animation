...
function deserialize(points) {
    ...
    for (let i = 0; i < n; i++) {
        g.vertices.push(new THREE.Vector3(spline[i * 2], spline[i * 2 + 1], spline[i * 2 + 1]));
    }

    let curve = new THREE.CatmullRomCurve3(g.vertices);
    const geometry = new THREE.Geometry();
    const splinePoints = curve.getPoints(1);
...
function addPoints() {
    points = deserialize(spline).geometry; // 
    // makeSquare(3);
...
function makeSquare(a) {
    ...
    // g.vertices.push(new THREE.Vector3(-a, -a, 0));
    // g.vertices.push(new THREE.Vector3(a, -a, 0));
    // g.vertices.push(new THREE.Vector3(a, a, 0));
    // g.vertices.push(new THREE.Vector3(-a, a, 0));
    // g.vertices.push(new THREE.Vector3(-a, -a, 0));
